...
class BitVectorSmall {
  ...
  public:
    ...
    static element_t log2(register bitvector_t x);

    inline element_t highest_bit() const { return highest_bit(_bitvector); }
    static element_t highest_bit(register bitvector_t x);
...
BitVectorSmall<uint_t>::log2(register bitvector_t x) {
...
template<class uint_t>
unsigned int
BitVectorSmall<uint_t>::cardinality(register bitvector_t x) {
  x = COUNT(x,0);
  x = COUNT(x,1);
  x = COUNT(x,2);
  if((sizeof(bitvector_t) * 8) > 8) {
...
template<class uint_t>
typename BitVectorSmall<uint_t>::element_t
BitVectorSmall<uint_t>::highest_bit(register bitvector_t x) {
  x |= (x >> 1);
  x |= (x >> 2);
  x |= (x >> 4);
  if(capacity() > 8) {
...
class BvHash {
  public:
    inline std::size_t operator()(const Tbv& aBv) const { return aBv.hashvalue(); }
};
